...
class AwsConnector(BaseConnector):
    SOURCE_NAME = "AWS"
    # Güncel ve çalışan ana giriş sayfası:
    START_URL = "https://aws.amazon.com/startups/"
    forbidden_terms = []

    def __init__(self, fetcher: BaseFetcher, cleaner: BaseCleaner | None = None):
        super().__init__(cleaner=cleaner)
        self.fetcher = fetcher
        self.base_domain = "https://aws.amazon.com"
        self.source_name = "AWS"

    async def fetch(self) -> list[dict]:
        print(f"{self.source_name}: AWS Activate ve girişimci destekleri taranıyor...\n")
        programs = []
        llm_client = get_llm_client()
        gorulen_linkler = set()
        
        target_patterns = ["/activate", "/startups/programs", "/startups/credits"]
        ...
        for link_etiketi in tum_linkler:
            try:
                ...
                if full_url in gorulen_linkler or "aws.amazon.com" not in full_url:
                    continue
                ...
                title = soup_detail.title.string.replace("AWS - ", "").strip() if soup_detail.title else (baslik or "AWS Support Program")

...
async def test_load_delegates_to_repo_get_session():
    ...
    with patch("backend.agents.memory.repo.get_session", return_value=fake_state) as get_session:
...
async def test_load_returns_none_when_no_session():
    agent = MemoryAgent()
    with patch("backend.agents.memory.repo.get_session", return_value=None):
...
async def test_save_delegates_to_repo_save_session_with_correct_state():
    ...
    with patch("backend.agents.memory.repo.save_session") as save_session:

  ...
  return (
    <section data-screen-label="Uygunluk kontrolü" style={{ height: "100%", overflowY: "auto" }}>
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "26px 32px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 18, background: "var(--surface)", border: "1px solid var(--border-subtle)", borderRadius: 16, padding: 20 }}>
          <div style={eligRingStyle(state, score)}>
            ...
            >
              <span style={{ fontSize: 17, fontWeight: 800, color: "var(--ink-900)" }}>{score}%</span>
            </div>
          </div>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 700, color: "var(--ink-900)", margin: 0 }}>{eligHeadline(state, program.conditions)}</h1>
            <div style={{ fontSize: 13, color: "var(--ink-400)", marginTop: 4 }}>{program.name}</div>
          </div>
        </div>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "space-between" }}>
                    <span style={{ fontSize: 13.5, fontWeight: 600, color: "var(--ink-900)" }}>{cond.text}</span>
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--ink-600)", marginTop: 3 }}>{cond.value}</div>
                  {cond.hint && (
                    <div style={{ fontSize: 12, color: s.col, marginTop: 7, lineHeight: 1.5, background: "var(--paper-100)", padding: "8px 10px", borderRadius: 9 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 26, padding: "16px 18px", background: "var(--paper-50)", border: "1px solid var(--border-subtle)", borderRadius: 14 }}>
          <Ms name={cta.icon} size={22} color="var(--terracotta-700)" />
          <p style={{ flex: 1, fontSize: 13.5, lineHeight: 1.55, color: "var(--ink-900)", margin: 0 }}>{cta.text}</p>
        </div>
